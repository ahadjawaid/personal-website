...
categories: [Reinforcement Learning, Deep Reinforcement Learning, Notes]
---
...
- **Reinforcement Learning:** A solution for managing complex, uncertain sequential decision-making.
...
- The environment encompasses states, actions, transition probabilities, and a reward function.

### States: Specific configurations of the environment
- A *state* provides a comprehensive description of the environment.

- **State Space:** An combiniation of all possible variables and values that can characterize the environment.
...
- A *transition function* demonstrates how the environment changes in response to an action. 
...
- **Bandit Environment:** An environment with a single non-terminal state, named 'bandit' as an analogy to a slot machine that can empty your pockets as bandits do.
...
-  The markov assumption is useful as it allows us to keep the number of variables in a state to a minimum since the more variables you add, the longer it will take the agent to learn.. This is advantageous when designing an agent capable of learning a task. However, completely adhering to the markov assumption may be impractical or even impossible. 
...
- A time step limit is often added to a continuing task to let the agent maximize the return.

#### Horizions: 
- **Planning Horizon:** It's the temporal scale of the task from the agent's perspective.
